...
The second argument to `defsc` is the props (short for properties) - a map of data passed in by the parent component, and normally derived based on the component's query. They are the same as https://reactjs.org/docs/components-and-props.html[React props], with few noteworthy additions:
...
****
We use qualified keywords a lot to avoid name conflicts and communicate ownership. If this is new to you, here is a brief summary of how to use them.
...
(let [{make :car/make, selected? :ui/selected?} props]
  (println make selected?))
...
When we require a namespace, we can give it an alias as in `(:require [my.long.ns :as myns])` and we can use the alias in the qualified keywords - the trick is to use double colon instead of a single colon in front of it, i.e. `::<ns alias>/kwd`. And if we leave out the alias, as in `::kwd`, it expands to the current ns, i.e. to `:<current ns>/kwd`.
...
For a _data entity_, an ident(ifier) is akin to a self-contained foreign key in SQL: it contains the (unique) name of an entity's ID property and its value, in a 2-element vector. For example: `[:person/id 123]`.
...
<3> Thanks to Root's initial state, the top-level key `:current-user` is added;
    and because the child has an ident, its state is normalized and we end up
    with an ident pointing to the actual data
<4> CurrentUser's data ends up normalized in the `:component/id` table and it is the
...
Mutations can be local-only (client-side) or local and remote (though there does not need to be any local behavior defined). Even though mutation usage looks like a function call, it is not. What `transact!` expects is a sequence of _data_:
...
Notice that idents of both `Person` and `Address` were used to put the data into the correct "tables". If `Address` lacked an ident, its data would stay denormalized inside the person just as it is in the input data. (The top component passed to `merge-component!` must always have an ident.)
...
When to call `load!`? Fulcro does not load any data from the server for you, you have to `load!` it yourself. The main options for when to do that are:

. When your {url-book}#_loading_something_into_the_db_root[application is starting]
. In an event handler (e.g. onClick)
. When a component is mounted, using React's `:componentDidMount` - though this is suboptimal and can result in loading cascades (A mounts and loads its data; after it gets the data, its child B is mounted and loads its data, ...); a better option is leveraging Fulcro's deferred routing
...
(defsc AccountList [_ {:account-list/keys [accounts]}]
 ;; Note: In practice, this would be a UI-only comp. with no query
...
```
====
<1> We load! using Account's query because it defines the data we want for each of the accounts
<2> We instruct load! to place the data where our UI expects it, i.e. inside the AccountList component instead of at the root of the client DB (Note: It would be more robust to use `(conj  (comp/get-ident AccountList) :account-list/accounts)` in case that we ever change the ident...)
...
. Clone the https://github.com/fulcrologic/fulcro/tree/main/src/todomvc/fulcro_todomvc[Fulcro Todo MVC] application, study it, and try to extend and change it. Some ideas:
